...
import { wordmarkLogo } from '@/lib/logo';
...
function PawLink({
  ...
  return (
    <li className="flex items-center gap-2">
      <Paw
        className="h-3.5 w-3.5 shrink-0 text-pool/45"
    ...
    <header className="fixed inset-x-0 top-3 z-40 px-3 sm:top-4 sm:px-4">
      ...
      >
        <a href="#home" className="flex shrink-0 items-center" aria-label="PawSure — home">
          <img src={wordmarkLogo} alt="PawSure" className="block h-7 w-auto sm:h-8" />
        </a>

        <ul className="hidden items-center gap-5 lg:flex">
          {nav.map((l, i) => (
            <PawLink key={l.label} {...l} tilt={i % 2 ? 16 : -14} />
          ))}
        </ul>
